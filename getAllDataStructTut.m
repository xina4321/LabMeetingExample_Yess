function [allData] = getAllDataStructTut(datFolder, masterSheet, task)
...
for ii = 1:length(datDirs) %why use ii as iterator variable rather than i? 
    ...
    for jj = 1: %how to know how many subjects? your code here
        ...
        %this could be done with a for loop or a cellfun
        %key function: strcmp() cellfun()
        ...
        %output! 
        % key function: strcmp(), cellfun()
        % to avoid cellfun, loop the task list and get an indicator
        % variable out for whether or not the target task was done
